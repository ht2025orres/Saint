...
export class AprobacionComponent implements OnInit {
  ...
  verificarMostrarDepartamento(): void {
    ...
    // 40 - aprobar inconsistencia lider
    this.esLider = this.authService.hasAnyPermission([7, 40]);
...
verificarRolLogistica(): void {
  ...
  // 43 - aprobar inconsistencia logistica
  this.mostrarAccionTomar = this.authService.hasAnyPermission([43]);
}
...
abrirModalDetalles(inconsistencia: any): void {
  ...
  this.inconsistenciasService.obtenerTiemposProceso(inconsistencia.id_inconsistencia || inconsistencia.id).subscribe({
    next: (res: any) => {
      ...
      // Validar si el usuario tiene algún permiso de aprobación (40-47 o el 7 legacy)
      const esAprobador = this.authService.hasAnyPermission([7, 40, 41, 42, 43, 44, 45, 46, 47]);
            ...
            <div id="pop-form-aprobar" style="display:none; background:#F0FDF4; border:1px solid #BBF7D0; border-radius:8px; padding:16px; margin-bottom:12px;">
              ...
              <div style="display:flex; gap:8px; justify-content:flex-end; margin-top:8px;">
                ...
                <button onclick="
                  ...
                  ${!tieneAccionPrevia ? `if (!accion) { alert('La acción a tomar es obligatoria.'); return; }` : ''}
                  ${estadoOrdenInvalido ? `if (!estadoOrden) { alert('El estado de la OP es obligatorio.'); return; }` : ''}
...
ponerEnEspera(inco: any): void {
  // 1. Verificar que el usuario tiene el acceso de Logística (rol o permiso)
  const rolesUsuario: string[] = (this.authService.user.roles || []).map((rol: any) => String(rol).toLowerCase());
  const permisosUsuario: string[] = (this.authService.user.permissions || []).map((p: any) => String(p).toLowerCase());
  const accesosUsuario = [...rolesUsuario, ...permisosUsuario];

  const esRolLogistica = accesosUsuario.some(acceso => acceso === 'logística (inconsistencias)' || acceso === 'logistica (inconsistencias)' || acceso === 'aprobar inconsistencia logistica');
