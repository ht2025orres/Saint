...
export class SolicitudComercialService {
  ...
  async parsePDF(file: File): Promise<any> {
    ...
    const closeBlock = () => {
      if (currentBlockHtml) {
        finalHtml += `<fieldset style="border:1px solid #cbd5e1; border-radius:8px; padding:15px; margin-bottom:20px; background:#f8fafc; position:relative; min-height:100px;">
          ...
          <div style="position:relative; height:${Math.max(100, Math.abs(blockYTop - lastY) * scale)}px;">${currentBlockHtml}</div>
        </fieldset>`;
        currentBlockHtml = '';
        blockYTop = 0;
      }
    ...
    allRawLines.forEach((line, idx) => {
      const txtLineFull = line.items.map(it => it.str).join(' ');
      ...
      let score = 0;
      kwds.forEach(k => { if (txtLineLow.includes(k)) score++; });
      const capsCount = line.items.filter(it => it.str === it.str.toUpperCase() && it.str.length > 1).length;
      const isHeader = score >= 3 || (score >= 2 && capsCount >= 4);

      if (isHeader) {
        closeBlock(); closeTable();
        isInsideTable = true;
        globalColBounds = line.items.map(it => it.xCoord).sort((a, b) => a - b);

        currentTableHtml += `<tr style="background:#f8fafc; border-bottom:2px solid #cbd5e1;">`;
        globalColBounds.forEach(bound => {
          const item = line.items.find(it => Math.abs(it.xCoord - bound) < 5);
      ...
      if (isInsideTable) {
        const numDensity = line.items.filter(it => /[\d\.,]{2,}/.test(it.str)).length;
        const isFooterKeyword = footerKwds.some(fk => txtLineLow.includes(fk));
        ...
        if (isFooterKeyword && numDensity < 2) {
          ...
          line.items.forEach(it => {
            ...
            globalColBounds!.forEach((bound, colIdx) => {
      ...
      // 3. POSICIONAMIENTO ABSOLUTO (Cabeceras y Datos Fieles)
      if (txtLineLow.length > 1) {
        if (blockYTop === 0) blockYTop = line.y;

        // Renderizado individual por cada palabra para asegurar posicionamiento exacto
        line.items.forEach(it => {
          const relY = Math.abs(it.transform[5] - blockYTop) * scale;
          const s = `position:absolute; left:${it.xCoord}px; top:${relY}px; font-size:0.8rem; color:#2d3748; white-space:nowrap; font-family:'Courier New', monospace;`;
          currentBlockHtml += `<span style="${s}">${it.str}</span>`;
        });
        lastY = line.y;

        // Si hay un salto vertical grande, forzamos cierre para evitar solapamientos
    ...
    const structuredData = { cabecera: { numero_oc: '', cliente_nombre: '', nit: '' }, items: [] };
    const fullText = allRawLines.map(l => l.items.map(it => it.str).join(' ')).join('\n');
    const nitM = fullText.match(/(NIT|IDENTIFICACION|RUC):?\s*([\d\.\-]{7,15})/i);
    if (nitM) structuredData.cabecera.nit = nitM[2].trim().replace(/\./g, '');
    const ocM = fullText.match(/(NUMERO|PEDIDO|ORDEN|OC):?\s*([A-Z0-9\-]{4,20})/i);
    if (ocM) structuredData.cabecera.numero_oc = ocM[2].trim();
    const cliM = fullText.match(/(SEÑORES|CLIENTE|NOMBRE|PROVEEDOR):?\s*([A-ZÑ\s\.]{5,60})/i);
