...
export class ProyectosComponent implements OnInit {
  ...
  get calendarioPlano(): CalendarioDia[] {
    return this.calendarioDias.reduce((acc, semana) => acc.concat(semana), [] as CalendarioDia[]);
  }
  ...
  get coloresParticipantesFlujo(): Record<number, string> {
    return this.participantesFlujoModal.reduce((acc, participante) => {
      acc[participante.id] = this.getColorPorId(participante.id);
      return acc;
  ...
  get inicialesParticipantesFlujo(): Record<number, string> {
    return this.participantesFlujoModal.reduce((acc, participante) => {
      acc[participante.id] = this.getInicialesResponsable(participante.id);
      return acc;
    }, {} as Record<number, string>);
  }
